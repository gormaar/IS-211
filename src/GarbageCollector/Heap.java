...
/**
 * This class is the memory model for the exam. Memory is represented by the int
 * array memory. The memory is partitioned into blocks, which can be either
 * objects, or free (unallocated) memory.
 *
 * The memory blocks are adjacent, in other words adding the size of of an
 * object to its address yields the address of the next object.
 *
 * In this simplified context, all objects have two pointers and some data. The
 * pointers are integers, the indices of the objects they point to.
 *
 * The garbage collector adds a header to the front of the object, with the data
 * that is needed by the gc algorithm.
 *
 * The layout is:
 * <table>
 * <tr><th>Field</th><th>Offset</th><th>Description</th></tr>
 * <tr><td>size</td><td>0</td><td>Total size of the object (including the
 * header)</td></tr>
 * <tr><td>Flag(s)</td><td>1</td><td>Used to mark objects</td></tr>
 * <tr><td>Next</td><td>2</td><td>Misnamed. It is a pointer that the garbage
 * collector may use.</td></tr>
 * <tr><td>ptr1</td><td>3</td><td>These two pointers simulate java class type
 * variables</td></tr>
...
public class Heap {
     ...
     * object creation.
     *
     * next i+2 points to the next element in linked lists ptr1 i+3 address,
     * i.e. a pointer to an object ptr2 i+4 another pointer these simulate class
     * type variables data i+5 object data (continues to i+size-1)
     *
     */
    int[] memory;
    /**
     * Simulates the root set, i.e. variables within the program that can always
     * be used to reach objects
     */
    int root;
    ...
    public void printMemoryMap(String header) {
        System.out.println("\n" + header);
        System.out.println("   Addr  Size   FLAG   Next   Ptr1   Ptr2   Data");
    ...
    public void printObject(int addr) {
        printObject(addr, null);
    }

    public void printObject(int addr, String prompt) {
        if (prompt != null) {
            System.out.println("\n" + prompt);
            System.out.println("   Addr  Size   FLAG   Next   Ptr1   Ptr2   Data");
        }
